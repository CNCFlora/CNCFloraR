...
for(mapaMapBiomas in MapBiomas_Brazil_set){
  ...
  if(AOO_area_s_centr_mun == 0){} else{
    ...
    if(is.na(sites_AOO)==T){
      ...
      final_output_mapbiomas_outras_lavouras_temporarias <- data.frame(Species = ESPECIE,
                                                                       Ameaca = "Outras lavouras temporárias",
                                                                       Area_AOO_sites_km2 = NA,
                                                                       Area_AOO_km2 = NA,
                                                                       Porcentagem_AOO = NA,
                                                                       AOO_util_km2 = NA,
      ...
      final_output_mapbiomas_mosaic_past_agric <- data.frame(Species = ESPECIE,
                                                             Ameaca = "Mosaico Agricultura e Pastagem",
                                                             Area_AOO_sites_km2 = NA,
                                                             Area_AOO_km2 = NA,
                                                             Porcentagem_AOO = NA,
                                                             AOO_util_km2 = NA,
      ...
      final_output_mapbiomas_alagado_pantanosa <- data.frame(Species = ESPECIE,
                                                             Ameaca = "Campo Alagado e Área Pantanosa",
                                                             Area_AOO_sites_km2 = NA,
                                                             Area_AOO_km2 = NA,
                                                             Porcentagem_AOO = NA,
                                                             AOO_util_km2 = NA,
      ...
      final_output_mapbiomas_outras_nao_florestais <- data.frame(Species = ESPECIE,
                                                                 Ameaca = "Outras Formações não Florestais",
                                                                 Area_AOO_sites_km2 = NA,
                                                                 Area_AOO_km2 = NA,
                                                                 Porcentagem_AOO = NA,
                                                                 AOO_util_km2 = NA,
      ...
      final_output_mapbiomas_outras_nao_vegetadas <- data.frame(Species = ESPECIE,
                                                                Ameaca = "Outras Áreas não Vegetadas",
                                                                Area_AOO_sites_km2 = NA,
                                                                Area_AOO_km2 = NA,
                                                                Porcentagem_AOO = NA,
                                                                AOO_util_km2 = NA,
      ...
      final_output_mapbiomas_outras_lavouras_perenes <- data.frame(Species = ESPECIE,
                                                                   Ameaca = "Outras Lavouras Perenes",
                                                                   Area_AOO_sites_km2 = NA,
                                                                   Area_AOO_km2 = NA,
                                                                   Porcentagem_AOO = NA,
                                                                   AOO_util_km2 = NA,
      ...
      # Exportando pontos coordenadas
      crs(sites)<-"+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
      shapefile(sites, paste0("/POINTS ", ESPECIE, ".shp"), overwrite=T)
      ...
      if(length(UTM_zone)==1){
        ...
        shapefile(sites_AOO_grid_AOO_1, paste0(getwd(), "/AOO_QuadOfGrid ", ESPECIE, ".shp"), overwrite = T)
      ...
      if(length(UTM_zone)==2){
        ...
        # Exportando shapefile AOO_QuadOfGrid
        shapefile(sites_grid_AOO, paste0(getwd(), "/AOO_QuadOfGrid ", ESPECIE, ".shp"), overwrite = T)
  ...
  if(AOO_area_s_centr_mun == 0){
    ...
    Ameacas <- output %>% dplyr::select(contains("Ameaca")) %>% unique()
...
if(AOO_area_s_centr_mun == 0){
  ...
  for(ano in periodo_MapBiomas_n){
    suppressMessages(anosMapBiomas<- anosMapBiomas %>%
                       add_column(assign(LETTERS[ano], ""),
                                  .name_repair = "unique"
                       )
    )
  }
...
write.csv(final_output, "SIG_land_use_TodosOsAnos_AOObyQuadOfGrid_output(CONABIO).csv", row.names = F)
