#' Overlay Analysis between occurrence records and historical series of cover land from MapBiomas
#'
#' Análise de sobreposição entre os registros de ocorrências e a série histórica de cobertura do solo do MapBiomas

overlayAnalysis_landCoverMapBiomas <- function(){
  ...
  OutputID <- readline("Output ID: ")

  switch(
    menu(

      c("Records and SIG valid", "Records and SIG not checked")

    ) + 1,
    cat("Exit\n"),
    ValidationCheck <- data.frame(
  ...
  # Paths of land use maps from MapBiomas ####

  MapBiomas_Brazil_landCover_timeSeries_filesPath <- NULL
  for(ANO in 1985:2020){

    MapBiomas_Brazil_landCover_timeSeries_filesPath_ <- paste0("D:/MapBiomas_Brazil_6.0_30m/MapBiomas_Brazil_6.0_", ANO, "_30m.tif")
    MapBiomas_Brazil_landCover_timeSeries_filesPath <- rbind (MapBiomas_Brazil_landCover_timeSeries_filesPath, MapBiomas_Brazil_landCover_timeSeries_filesPath_)

  }


  # List of Species file (species_landCover-MapBiomas.csv) ####
  ...
  with_drive_quiet(
    drive_download(
      ...
      drive_get("species_landCover-MapBiomas.csv"),
      path = paste0(sub("Packages/CNCFloraR", "", getwd()), "CNCFlora_data/inputs/listOfSpecies_for_processing/species_landCover-MapBiomas.csv"),
      overwrite = TRUE
  ...
  listOfSpecies_localPath <- paste0(sub("Packages/CNCFloraR", "", getwd()), "CNCFlora_data/inputs/listOfSpecies_for_processing/species_landCover-MapBiomas.csv")
  ...
  while(Answer == "" |
    ...
    if(Answer == "n"){
      ...
      message("Aborting...\n\n Update the file: ", listOfSpecies_localPath)
  ...
  listOfSpecies_n <- 1:as.numeric(nrow(listOfSpecies))


  # Declare final objects ####

  final_output <- NULL
  final_output_comb <- NULL


  # Loop start

  for(j in listOfSpecies_n){

    for(MapBiomas_Brazil_landCover_year_filepath in MapBiomas_Brazil_landCover_timeSeries_filesPath){
      ...
      rm(
        list = setdiff(
          ...
          c(
            ...
            "OutputID",
            "ValidationCheck",
            "MapBiomas_Brazil_landCover_Map",
            "MapBiomas_Brazil_landCover_timeSeries_filesPath",
            "MapBiomas_Brazil_landCover_year_filepath",
            "listOfSpecies",
            "listOfSpecies_n",
            "final_output",
            "final_output_comb",
      ...
      # Year of the MapBiomas map ####

      MapBiomas_Brazil_landCover_yearMap <-
        str_extract(
          MapBiomas_Brazil_landCover_year_filepath,
          "[0-9][0-9][0-9][0-9]"
        )
      ...
      # MapBiomas raster map of a year ####

      MapBiomas_Brazil_landCover_Map <- raster(MapBiomas_Brazil_landCover_year_filepath)


      # Name of the species ####

      Species <- listOfSpecies[j, 1]
      ...
      # Scraping data of occurrence records from old System ####

      coords_df <- scrap_occurrenceRecords_from_oldSystem(input)


      # Screening of valid occurrence records ####

      if(ValidationCheck$Validation == "Checked"){

        coords_df_valid <-
          coords_df %>%
          dplyr::filter(Validade == ValidationCheck$records & Validade_SIG == ValidationCheck$SIG)

      }


      # Verification of degree zone and UTMzone ####

      coords_df_valid <- verification_degree_and_UTMzones(coords_df_valid)


      # Number of species vector ####
      ### O mesmo objeto está presente na função `verification_degree_and_UTMzones` e `delete_empty_coordinates`
      #coords_df_valid_n_row <- as.numeric(1:(length(coords_df_valid$Degree_zone)))


      # Deleting empty coordinates ####

      coords_df_valid <- delete_empty_coordinates(coords_df_valid)


      # Convert UTMzone string to CRS statement ####

      coords_df_valid <- convert_UTMzone_to_CRSstatement(coords_df_valid)


      # Deletion of coordinates with municipality centroid precision ####

      coords_df_valid_AOO <- coords_df_valid %>%
        filter (!str_detect(precision, "[C|c]entr[o|ó]ide"))


      # Creation of the geographic coordinates dataframe (AOO) ####
      ...
      if(dplyr::count(coords_df_valid_AOO) == 0){
        ...
        sites_AOO <-
          SpatialPointsDataFrame(
            coords_df_valid_AOO[,1:2],
            coords_df_valid_AOO[,3:11]
          )

      }


      # Creation of the geographic coordinates dataframe (EOO) ####
      ...
      if(dplyr::count(coords_df_valid) == 0){
        ...
        sites <-
          SpatialPointsDataFrame(
            coords_df_valid[,1:2],
            coords_df_valid[,3:11]
          )

      }


      # Difference between AOO with/without municipality centroid ####
      ...
      if(AOO_area_s_centr_mun == 0){} else{
        ...
        if(
          ...
          nrow(unique(data.frame(
            ...
            lat = coords_df_valid$lat,
            lon = coords_df_valid$lon
          ...
          ## Crop EOO in UTM zones, define CRS and calculate EOO area ####

          area_EOO_km2 <- cropEOO_defineCRS_calculateArea(EOO)
          ...
          if(area_EOO_km2 <= 40000){
            ...
            if(exists("area_km2_EOO_18N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_18N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_18N) <- "+proj=utm +zone=18 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_18N_ymin), roundUpFirstDecimalPlace(bbox_18N_ymax), by=0.1)
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_18N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_18N, get(paste("EOO_18N","_", i, sep = ""))))
              ...
              analiseEOO_18N <-
                analise_EOO_18N_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))

            }

            if(exists("area_km2_EOO_18S")){

              MapBiomas_Brazil_landCover_Map_EOO_18S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_18S) <- "+proj=utm +zone=18 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_18S_ymin), roundUpFirstDecimalPlace(bbox_18S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_18S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_18S, get(paste("EOO_18S","_", i, sep = ""))))
              ...
              analiseEOO_18S <- analise_EOO_18S_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            ...
            if(exists("area_km2_EOO_19N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_19N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_19N) <- "+proj=utm +zone=19 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_19N_ymin), roundUpFirstDecimalPlace(bbox_19N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_19N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_19N, get(paste("EOO_19N","_", i, sep = ""))))
              ...
              analiseEOO_19N <- analise_EOO_19N_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            ...
            if(exists("area_km2_EOO_19S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_19S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_19S) <- "+proj=utm +zone=19 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_19S_ymin), roundUpFirstDecimalPlace(bbox_19S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_19S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_19S, get(paste("EOO_19S","_", i, sep = ""))))
              ...
              analiseEOO_19S <- analise_EOO_19S_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            ...
            if(exists("area_km2_EOO_20N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_20N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_20N) <- "+proj=utm +zone=20 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_20N_ymin), roundUpFirstDecimalPlace(bbox_20N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_20N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_20N, get(paste("EOO_20N","_", i, sep = ""))))
              ...
              analiseEOO_20N <- analise_EOO_20N_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            ...
            if(exists("area_km2_EOO_20S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_20S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_20S) <- "+proj=utm +zone=20 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_20S_ymin), roundUpFirstDecimalPlace(bbox_20S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_20S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_20S, get(paste("EOO_20S","_", i, sep = ""))))
              ...
              analiseEOO_20S <-
                analise_EOO_20S_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))

            }
            if(exists("area_km2_EOO_21N")){
              MapBiomas_Brazil_landCover_Map_EOO_21N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_21N) <- "+proj=utm +zone=21 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_21N_ymin), roundUpFirstDecimalPlace(bbox_21N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_21N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_21N, get(paste("EOO_21N","_", i, sep = ""))))
              ...
              analiseEOO_21N <- analise_EOO_21N_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            ...
            if(exists("area_km2_EOO_21S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_21S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_21S) <- "+proj=utm +zone=21 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_21S_ymin), roundUpFirstDecimalPlace(bbox_21S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_21S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_21S, get(paste("EOO_21S","_", i, sep = ""))))
              ...
              analiseEOO_21S <- analise_EOO_21S_summ %>%
                group_by(value) %>%
                dplyr::summarise(n=sum(n), coverage_fraction=sum(sum))
            }
            ...
            if(exists("area_km2_EOO_22N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_22N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_22N) <- "+proj=utm +zone=22 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_22N_ymin), roundUpFirstDecimalPlace(bbox_22N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_22N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_22N, get(paste("EOO_22N","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_22S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_22S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_22S) <- "+proj=utm +zone=22 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_22S_ymin), roundUpFirstDecimalPlace(bbox_22S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_22S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_22S, get(paste("EOO_22S","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_23N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_23N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_23N) <- "+proj=utm +zone=23 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_23N_ymin), roundUpFirstDecimalPlace(bbox_23N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_23N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_23N, get(paste("EOO_23N","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_23S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_23S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_23S) <- "+proj=utm +zone=23 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_23S_ymin), roundUpFirstDecimalPlace(bbox_23S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_23S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_23S, get(paste("EOO_23S","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_24N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_24N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_24N) <- "+proj=utm +zone=24 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_24N_ymin), roundUpFirstDecimalPlace(bbox_24N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_24N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_24N, get(paste("EOO_24N","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_24S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_24S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_24S) <- "+proj=utm +zone=24 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_24S_ymin), roundUpFirstDecimalPlace(bbox_24S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_24S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_24S, get(paste("EOO_24S","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_25N")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_25N <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_25N) <- "+proj=utm +zone=25 +north +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_25N_ymin), roundUpFirstDecimalPlace(bbox_25N_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_25N","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_25N, get(paste("EOO_25N","_", i, sep = ""))))
            ...
            if(exists("area_km2_EOO_25S")){
              ...
              MapBiomas_Brazil_landCover_Map_EOO_25S <- MapBiomas_Brazil_landCover_Map
              crs(MapBiomas_Brazil_landCover_Map_EOO_25S) <- "+proj=utm +zone=25 +south +datum=WGS84"
              ...
              seq_y <- seq(roundDownFirstDecimalPlace(bbox_25S_ymin), roundUpFirstDecimalPlace(bbox_25S_ymax), by=0.1)
              seq_y_num_total <- length(seq_y)
              seq_y_num <- 1:length(seq_y)
              seq_y_num <- seq_y_num[-seq_y_num_total]
              ...
              for(i in seq_y_num){
                ...
                assign(paste("analise_EOO_25S","_", i, "_sf", sep = ""), exact_extract(MapBiomas_Brazil_landCover_Map_EOO_25S, get(paste("EOO_25S","_", i, sep = ""))))
            ...
            # Filtering classes of land cover  ####
        ...
        if(is.na(sites_AOO)==T){
          ...
          final_output_mapbiomas_urbano <- data.frame(Species=Species, Ameaca="Infraestrutura Urbana", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_urbano)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_urbano)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_mineracao <- data.frame(Species=Species, Ameaca="Mineração", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_mineracao)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_mineracao)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_silvicultura <- data.frame(Species=Species, Ameaca="Silvicultura", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_silvicultura)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_silvicultura)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_outras_lavouras_temporarias <- data.frame(Species=Species, Ameaca="Outras lavouras temporárias", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_outras_lavouras_temporarias)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_outras_lavouras_temporarias)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_soja <- data.frame(Species=Species, Ameaca="Soja", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_soja)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_soja)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_cana <- data.frame(Species=Species, Ameaca="Cana-de-açúcar", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_cana)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_cana)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_lavouras_perenes <- data.frame(Species=Species, Ameaca="Lavouras perenes", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_lavouras_perenes)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_lavouras_perenes)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_pastagem <- data.frame(Species=Species, Ameaca="Pastagem", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_pastagem)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_pastagem)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_mosaic_past_agric <- data.frame(Species=Species, Ameaca="Mosaico Agricultura e Pastagem", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_mosaic_past_agric)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_mosaic_past_agric)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_floresta <- data.frame(Species=Species, Ameaca="Floresta", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_floresta)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_floresta)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_savana <- data.frame(Species=Species, Ameaca="Savana", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_savana)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_savana)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_mangue <- data.frame(Species=Species, Ameaca="Mangue", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_mangue)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_mangue)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_alagado_pantanosa <- data.frame(Species=Species, Ameaca="Campo Alagado e Área Pantanosa", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_alagado_pantanosa)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_alagado_pantanosa)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_campos <- data.frame(Species=Species, Ameaca="Formação Campestre", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_campos)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_campos)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_outras_nao_florestais <- data.frame(Species=Species, Ameaca="Outras Formações não Florestais", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_outras_nao_florestais)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_outras_nao_florestais)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_lavouras_temporarias <- data.frame(Species=Species, Ameaca="Lavoura Temporária", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_lavouras_temporarias)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_lavouras_temporarias)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_praia_duna_areal <- data.frame(Species=Species, Ameaca="Praia, Duna e Areal", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_praia_duna_areal)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_praia_duna_areal)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_outras_nao_vegetadas <- data.frame(Species=Species, Ameaca="Outras Áreas não Vegetadas", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_outras_nao_vegetadas)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_outras_nao_vegetadas)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_nao_observado <- data.frame(Species=Species, Ameaca="Não Observado", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_nao_observado)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_nao_observado)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_afloramento_rochoso <- data.frame(Species=Species, Ameaca="Afloramento Rochoso", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_afloramento_rochoso)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_afloramento_rochoso)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_aquicultura <- data.frame(Species=Species, Ameaca="Aquicultura", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_aquicultura)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_aquicultura)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_apicum <- data.frame(Species=Species, Ameaca="Apicum", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_apicum)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_apicum)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_rio_lago_oceano <- data.frame(Species=Species, Ameaca="Rio, Lago e Oceano", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_rio_lago_oceano)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_rio_lago_oceano)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_arroz <- data.frame(Species=Species, Ameaca="Arroz", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_arroz)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_arroz)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_cafe <- data.frame(Species=Species, Ameaca="Café", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_cafe)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_cafe)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_citrus <- data.frame(Species=Species, Ameaca="Citrus", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_citrus)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_citrus)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_outras_lavouras_perenes <- data.frame(Species=Species, Ameaca="Outras Lavouras Perenes", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_outras_lavouras_perenes)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_outras_lavouras_perenes)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

          final_output_mapbiomas_restinga_arborizada <- data.frame(Species=Species, Ameaca="Restinga Arborizada", Area_AOO_sites_km2=NA, Area_AOO_km2=AOO_area_c_centr_mun, Porcentagem_AOO=NA, AOO_util_km2=NA, Area_EOO_sites_km2=NA, Area_EOO_km2=0, Porcentagem_EOO=NA, Fonte="MapBiomas")
          colnames(final_output_mapbiomas_restinga_arborizada)[c(3,5,7,9)] <- paste0(colnames(final_output_mapbiomas_restinga_arborizada)[c(3,5,7,9)], "_", MapBiomas_Brazil_landCover_yearMap)

        } else{


          ## Create objects of UTMzone and CRS ####
          ...
          if(length(UTM_zone) == 1){
            ...
            MapBiomas_Brazil1 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 2){
            ...
            # Converter raster para Zona UTM/WGS84
            MapBiomas_Brazil2 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 3){
            ...
            # Converter raster para Zona UTM/WGS84
            MapBiomas_Brazil3 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 4){
            ...
            # Converter raster para Zona UTM/WGS84
            MapBiomas_Brazil4 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 5){
            ...
            MapBiomas_Brazil5 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 6){
            ...
            MapBiomas_Brazil6 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 7){
            ...
            MapBiomas_Brazil7 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 8){
            ...
            MapBiomas_Brazil8 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 9){
            ...
            MapBiomas_Brazil9 <- MapBiomas_Brazil_landCover_Map
          ...
          if(length(UTM_zone) == 10){
            ...
            MapBiomas_Brazil10 <- MapBiomas_Brazil_landCover_Map
        ...
        Ameacas <-
          output %>%
          dplyr::select(Ameaca)
        output_sites_AOO_km2_total <- output %>% dplyr::summarise(sum(get(paste0("Area_AOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap))))
        if(

          output[1,7] == "Análise dispensada" | is.na(output[1,7]) == T
          ...
          output_sites_EOO_km2_total <- output %>% dplyr::summarise(sum(get(paste0("Area_EOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap))))
        ...
        colnames(output_sites_AOO_km2_total)<-paste0("Area_AOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap)
        colnames(output_sites_EOO_km2_total)<-paste0("Area_EOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap)
        assign(paste0("output_Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap, "_total"), output %>% dplyr::summarise(sum(get(paste0("Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap)))))
        assign(paste0("output_Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap, "_total"), output %>% dplyr::summarise(sum(get(paste0("Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap)))))
        assign(paste0("output_Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap, "_total"), setNames(get(paste0("output_Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap, "_total")), paste0("Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap)))
        assign(paste0("output_Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap, "_total"), setNames(get(paste0("output_Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap, "_total")), paste0("Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap)))
        ...
        output <-
          output %>%
          dplyr::select(paste0("Area_AOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap), paste0("Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap), paste0("Area_EOO_sites_km2_", MapBiomas_Brazil_landCover_yearMap), paste0("Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap))

        output <- rbind(output, data.frame(Area_AOO_sites_km2=output_sites_AOO_km2_total, Porcentagem_AOO=get(paste0("output_Porcentagem_AOO_", MapBiomas_Brazil_landCover_yearMap, "_", "total")), Area_EOO_sites_km2=output_sites_EOO_km2_total, Porcentagem_EOO=get(paste0("output_Porcentagem_EOO_", MapBiomas_Brazil_landCover_yearMap, "_", "total"))))
      ...
      if(AOO_area_s_centr_mun == 0){} else{
        ...
        cat(crayon::green(paste0(Species, ": Ano ", MapBiomas_Brazil_landCover_yearMap, " concluído\n")))
    ...
    if(AOO_area_s_centr_mun == 0){
      ...
      final_output_comb <-
        final_output_comb %>%
        add_row(Species = Species, Area_AOO_km2 = AOO_area_c_centr_mun, AOO_util_km2 = AOO_area_s_centr_mun, Area_EOO_km2 = NA, Fonte = "MapBiomas")

      cat(crayon::green(paste0(Species, ": Sem registros úteis\n")))

    } else{

      final_output <- data.frame(Species = Species, Ameaca = rbind(Ameacas, "Total"), Area_AOO_km2 = AOO_area_c_centr_mun, AOO_util_km2 = AOO_area_s_centr_mun, Area_EOO_km2 = area_EOO_km2, final_output, Fonte = "MapBiomas")
      final_output_comb <- rbind(final_output_comb, final_output)
      final_output <- NULL

      cat(crayon::green(paste0(Species, ": concluído\n")))
  ...
  write.csv(
    ...
    final_output_comb,
    paste0("SIG_landCover_MapBiomas_output(", outputID, ").csv"), row.names = F
